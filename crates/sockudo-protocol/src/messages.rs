...
pub struct PusherMessage {
    ...
    /// Opaque per-channel continuity token for durable history and recovery.
    /// Changes only when the server can no longer prove continuity for the channel stream.
    #[serde(skip_serializing_if = "Option::is_none")]
