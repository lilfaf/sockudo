...
impl ServerOptions {
    ...
    pub async fn override_from_env(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(driver_str) = std::env::var("PUSH_STORAGE_DRIVER") {
            self.push.storage_driver = parse_driver_enum(
                driver_str,
                self.push.storage_driver.clone(),
                "Push Storage Backend",
            );
        }
        if let Ok(driver_str) = std::env::var("PUSH_QUEUE_DRIVER") {
            self.push.queue_driver = parse_driver_enum(
                driver_str,
                self.push.queue_driver.clone(),
                "Push Queue Backend",
            );
        }
        self.push.fcm_enabled = parse_bool_env("PUSH_FCM_ENABLED", self.push.fcm_enabled);
        self.push.apns_enabled = parse_bool_env("PUSH_APNS_ENABLED", self.push.apns_enabled);
        self.push.webpush_enabled =
            parse_bool_env("PUSH_WEBPUSH_ENABLED", self.push.webpush_enabled);
        self.push.hms_enabled = parse_bool_env("PUSH_HMS_ENABLED", self.push.hms_enabled);
        self.push.wns_enabled = parse_bool_env("PUSH_WNS_ENABLED", self.push.wns_enabled);
        if let Some(key) = parse_env_optional::<String>("PUSH_CREDENTIAL_ENCRYPTION_KEY") {
            self.push.credential_encryption_key = Some(key);
        }
        self.push.fanout_fast_threshold = parse_env::<u64>(
            "PUSH_FANOUT_FAST_THRESHOLD",
            self.push.fanout_fast_threshold,
        );
        self.push.fanout_shard_size =
            parse_env::<u64>("PUSH_FANOUT_SHARD_SIZE", self.push.fanout_shard_size);
        self.push.fanout_sync_threshold = parse_env::<u64>(
            "PUSH_FANOUT_SYNC_THRESHOLD",
            self.push.fanout_sync_threshold,
        );
        self.push.backpressure_lag_threshold_secs = parse_env::<u64>(
            "PUSH_BACKPRESSURE_LAG_THRESHOLD_SECS",
            self.push.backpressure_lag_threshold_secs,
        );
        self.push.publish_status_ttl_days = parse_env::<u64>(
            "PUSH_PUBLISH_STATUS_TTL_DAYS",
            self.push.publish_status_ttl_days,
        );
        self.push.circuit_breaker.failure_threshold = parse_env::<u32>(
            "PUSH_FAILURE_THRESHOLD",
            self.push.circuit_breaker.failure_threshold,
        );
        self.push.scheduler_interval_secs = parse_env::<u64>(
            "PUSH_SCHEDULER_INTERVAL_SECS",
            self.push.scheduler_interval_secs,
        );
        self.push.stale_device_max_age_days = parse_env::<u64>(
            "PUSH_STALE_DEVICE_MAX_AGE_DAYS",
            self.push.stale_device_max_age_days,
        );
        self.push.analytics_enabled =
            parse_bool_env("PUSH_ANALYTICS_ENABLED", self.push.analytics_enabled);
        self.push.default_quotas.acceptance_rps = parse_env::<u64>(
            "PUSH_DEFAULT_ACCEPTANCE_RPS",
            self.push.default_quotas.acceptance_rps,
        );
        self.push.default_quotas.delivery_quota_daily = parse_env::<u64>(
            "PUSH_DEFAULT_DELIVERY_QUOTA_DAILY",
            self.push.default_quotas.delivery_quota_daily,
        );
        self.push.default_quotas.fanout_max = parse_env::<u64>(
            "PUSH_DEFAULT_FANOUT_MAX",
            self.push.default_quotas.fanout_max,
        );
        self.push.default_quotas.inflight_max = parse_env::<u64>(
            "PUSH_DEFAULT_INFLIGHT_MAX",
            self.push.default_quotas.inflight_max,
        );
